...
show_only_frames = [0, 200] # show only frames in interval for debugging 51
...
while True:
    try:
        ...
        if (configs_det.use_labels_as_objects==True):
            ...
            if 'detect_objects' in exec_list:
                ...
                if 'perform_tracking' in exec_list:
                    detections = load_object_from_file(results_path, data_filename, 'detections', cnt_frame)
                else:
        ...
        if 'measure_detection_performance' in exec_list:
            ...
            # load different data for final project vs. mid-term project
            if 'perform_tracking' in exec_list:
                det_performance = load_object_from_file(results_path, data_filename, 'det_performance', cnt_frame)
            else:
                ...
                det_performance = load_object_from_file(results_fullpath, data_filename, 'det_performance_' + configs_det.arch.rsplit('_', 1)[0] + '_' + str(configs_det.conf_thresh), cnt_frame)   
