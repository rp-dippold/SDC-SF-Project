...
def show_pcl(pcl):
    ...
    pcl_vis.create_window(window_name='Open3D', visible=True)
    ...
    pcl_vis.register_key_callback(262, lambda v: v.destroy_window())
    pcl_vis.run()
...
def bev_from_pcl(lidar_pcl, configs):
    ...
    lidar_pcl_cpy[:, 1] = np.int_(np.floor((lidar_pcl_cpy[:, 1] + np.abs(configs['lim_y'][0])) / bev_discret))
    ...
    intensity_idx = np.lexsort((-lidar_pcl_cpy[:, 3], lidar_pcl_cpy[:, 1], lidar_pcl_cpy[:, 0]))
    ...
    lidar_pcl_intensity = lidar_pcl_cpy[idx]
    ...
    intensity_map[np.int_(lidar_pcl_intensity[:, 0]), np.int_(lidar_pcl_intensity[:, 1])] = \
        lidar_pcl_intensity[:, 3] / (np.amax(lidar_pcl_intensity[:, 3])-np.amin(lidar_pcl_intensity[:, 3]))
    ...
    lidar_pcl_cpy = lidar_pcl_cpy[height_idx]
    _, idx = np.unique(lidar_pcl_cpy[:, :2], axis=0, return_index=True)
