...
def measure_detection_performance(detections, labels, labels_valid, min_iou=0.5):
    ...
    for label, valid in zip(labels, labels_valid):
        ...
        if valid: # exclude all labels from statistics which are not considered valid
            ...
            corners_lbl = tools.compute_box_corners(label.box.center_x, 
                                                    label.box.center_y, 
                                                    label.box.width, 
                                                    label.box.length, 
                                                    label.box.heading)
            ...
            for obj in detections:
                ...
                lbl_poly = Polygon(corners_lbl)
                det_poly = Polygon(corners_det)
                intersec_area = lbl_poly.intersection(det_poly).area
                union_area = lbl_poly.union(det_poly).area
