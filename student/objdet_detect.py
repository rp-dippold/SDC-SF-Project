...
def load_configs_model(model_name='darknet', configs=None):
    ...
    if model_name == "darknet":
        ...
        configs.pretrained_filename = os.path.join(configs.model_path, 'tools',
                                               'objdet_models', 'resnet', 
...
def detect_objects(input_bev_maps, model, configs):
    ...
    with torch.no_grad():  
        ...
        if 'darknet' in configs.arch:
            ...
            out_dec = decode(outputs['hm_cen'], outputs['cen_offset'], outputs['direction'],
                             outputs['z_coor'], outputs['dim'], K=configs.K).numpy()
            
            output_post = post_processing(out_dec, configs)
            detections = []
            for output in output_post:
                # detection attributes: x, y, z, h, w, l, yaw
                detection = output.get(1, [])
                for obj in detection:
                    x, y, z, h, w, l, yaw = obj[1:2][0], obj[2:3][0], \
                                            obj[3:4][0], obj[4:5][0], \
                                            obj[5:6][0], obj[6:7][0], \
                                            obj[7:8][0]
    ...
    if len(detections) > 0:
        ...
        for detection in detections:
            ...
            _, x_bev, y_bev, z, h, w_bev, l_bev, yaw = detection
            x = y_bev / configs.bev_height * (configs.lim_x[1] - configs.lim_x[0])
            y = (x_bev - configs.bev_width / 2.0) / configs.bev_width * (configs.lim_y[1] - configs.lim_y[0])
